...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!env.LEAD_OPTIN_WEBHOOK_URL) {
    return json({ error: "Missing LEAD_OPTIN_WEBHOOK_URL secret on Pages project." }, 500, cHeaders);
  }
  ...
  const forwardRes = await forwardJson(env.LEAD_OPTIN_WEBHOOK_URL, payload, env);
